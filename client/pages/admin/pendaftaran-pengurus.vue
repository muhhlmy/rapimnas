          ...
          <table class="w-full table-fixed">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-2 py-3 text-gray-700 break-words leading-snug flex-col">
                  <div>{{ item.name }}</div>
                  <div class="text-xs text-gray-500">{{ item.kta_name }}</div>
                </td>
...
const ALLOWED_CATEGORIES = ["KI WKUK", "KI WKU", "KI KOMTAP", "KI WAKOMTAP"];
...
const fetchParticipants = async () => {
  ...
  try {
    ...
    // Add filter for category based on active tab - ONLY ALLOWED CATEGORIES
    const categoryMap = {
      'WKUK': 'KI WKUK',
      'WKU': 'KI WKU',
      'KOMTAP': 'KI KOMTAP',
      'WAKOMTAP': 'KI WAKOMTAP'
    };

    if (activeTab.value !== 'All') {
      if (categoryMap[activeTab.value]) {
        params.append('filter', JSON.stringify({
          category: categoryMap[activeTab.value]
        }));
      }
    } else {
      // When "All" is selected, only show allowed categories
      params.append('filter', JSON.stringify({
